...
class ExtractTextFromPDF
{
    ...
    private $outputFilePath = '../output_files/extract_text_from_pdf/AboutFoxit.txt';
    private $baseUrl = 'https://servicesapi.foxitsoftware.cn/api';

    // Build URI for endpoint
    private function buildUri($endpoint)
    {
        return rtrim($this->baseUrl, '/') . '/' . ltrim($endpoint, '/');
    }

    // Get clientId and secretId from the credentials file
    ...
    // Extract text from PDF task
    private function extractTextFromPdfTask($inputFilePath, $mode = "extractText", $pageRange = "all")
    {
        $queryParams = [
    ...
    public function start()
    {
        try {
            ...
            $taskId = $this->extractTextFromPdfTask($this->inputFilePath);
            $docId = $this->pollForDocid($taskId);
            $this->downloadFileByDocid($docId, $this->outputFilePath);
            echo 'Extract text from PDF successfully!' . "\n";
        } catch (Exception $e) {
            echo $e->getMessage() . "\n";
        }
    }
...
// Run the extraction
$extractTextFromPdf = new ExtractTextFromPDF();
