...
class CreatePdfFromOthers {
    ...
    private $secretId = '';
    private $sn = 'testsn';
    private $inputFilePath = '../input_files/AboutFoxit.doc';
    private $outputFilePath = '../output_files/create_pdf_from_others/AboutFoxit.pdf';
    ...
    private function createPdfTask($inputFilePath, $format = 'word') {
        $queryParams = [
        ...
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, $this->buildUri('document/create') . '?' . http_build_query($params));
        curl_setopt($ch, CURLOPT_POST, true);
        curl_setopt($ch, CURLOPT_POSTFIELDS, [
            'format' => $format,
            'inputDocument' => new CURLFile($inputFilePath, 'application/msword', $filename),
        ]);
    ...
    public function start() {
        try {
            ...
            $taskId = $this->createPdfTask($this->inputFilePath);
...
$converter = new CreatePdfFromOthers();
$converter->start();
?>
