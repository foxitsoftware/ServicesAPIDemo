...
class Combine {
    ...
    public function __construct() {
        ...
        $this->secretId = '';
        $this->sn = 'testsn';
        $this->inputFilePath = '../input_files/combine.zip';
        $this->outputFilePath = '../output_files/combine/CombineResultFiles.pdf';
    ...
    private function combineTask($inputZipFile) {
        ...
        $file = new CURLFile($inputZipFile, 'application/zip', basename($inputZipFile));
        $postData = [
            'config' => $configString,
            'inputZipDocument' => $file
        ];

        $ch = curl_init($this->buildUri('document/combine') . '?' . http_build_query($params));
