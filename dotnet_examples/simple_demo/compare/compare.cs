...
namespace CompareCS
{
  ...
  class Compare
  {
    ...
    private const string input_file_path1 = "../input_files/test_base.pdf";
    private const string input_file_path2 = "../input_files/test_compared.pdf";
    ...
    public static void Start()
    {
      ...
      try
      {
        ...
        compare.GetCredentialsParams(Directory.GetCurrentDirectory() + "/foxit_cloud_api_credentials.json");
        string task_id = compare.ComparePDFTask(input_file_path1, input_file_path2);
        string doc_id = compare.PollForDocId(task_id);
                compare.DownLoadFileByDocId(doc_id, output_file_path);
        Console.WriteLine("Compare PDF files successfully!");
      }
      catch(RestException e)
      {
