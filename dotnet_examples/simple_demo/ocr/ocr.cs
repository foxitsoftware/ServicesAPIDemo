...
namespace OCRCS
{
    ...
    class OCR
    {
        ...
        private const string output_file_path = "../output_files/ocr/AboutFoxit_ocr.txt";
        ...
        private string OCRTask(string input_file)
        {
            ...
            string url = $"https://servicesapi.foxitsoftware.cn/api/document/ocr?sn={sn}&clientId={client_id}";
            ...
            formData.Add(new StringContent(output_format), "outputFormat");
        ...
        public static void Start()
        {
            ...
            {
                OCR ocr = new OCR();
                ocr.GetCredentialsParams(Directory.GetCurrentDirectory() + "/foxit_cloud_api_credentials.json");
                string task_id = ocr.OCRTask(input_file_path);
                string doc_id = ocr.PollForDocId(task_id);
                ocr.DownLoadFileByDocId(doc_id, output_file_path);
                Console.WriteLine("OCR PDF file successfully!");
            }
            catch (RestException e)
            {
