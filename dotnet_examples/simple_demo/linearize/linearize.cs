...
//
// This file contains an example to demonstrate how to use Foxit Cloud API to linearize pdf file.
...
namespace LinearizeCS
{
    ...
    class Linearize
    {
        ...
        private string LinearizePDFTask(string input_file_path)
        {
            ...
            string url = $"https://servicesapi.foxitsoftware.cn/api/document/linearize?sn={sn}&clientId={client_id}";
        ...
        public static void Start()
        {
            ...
            try
            {
                ...
                linearize.GetCredentialsParams(Directory.GetCurrentDirectory() + "/foxit_cloud_api_credentials.json");
                string task_id = linearize.LinearizePDFTask(input_file_path);
                string doc_id = linearize.PollForDocId(task_id);
                linearize.DownLoadFileByDocId(doc_id, output_file_path);
                Console.WriteLine("Linearize PDF successfully!");
            }
            catch (RestException e)
            {
