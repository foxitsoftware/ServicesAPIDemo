...
namespace ConvertPDFToOthersCS
{
    ...
    class ConvertPDFToOthers
    {
        ...
        private const string output_file_path = "../output_files/convert_pdf_to_others/Image.zip";
        ...
        private string ConvertPDFTask(string input_file_path, string format = "image")
        {
            string config = "{\r\n  \"dpi\": 96,\r\n  \"pageRange\": \"all\" \r\n}";
            ...
            string url = $"https://servicesapi.foxitsoftware.cn/api/document/convert?sn={sn}&clientId={client_id}";
        ...
        public static void Start()
        {
            ...
            try
            {
                ConvertPDFToOthers convert_pdf_to_others = new ConvertPDFToOthers();
                convert_pdf_to_others.GetCredentialsParams(Directory.GetCurrentDirectory() + "/foxit_cloud_api_credentials.json");
                string task_id = convert_pdf_to_others.ConvertPDFTask(input_file_path);
                string doc_id = convert_pdf_to_others.PollForDocId(task_id);
                convert_pdf_to_others.DownLoadFileByDocId(doc_id, output_file_path);
                Console.WriteLine("Convert PDF to image successfully!");
            }
            catch (RestException e)
            {
