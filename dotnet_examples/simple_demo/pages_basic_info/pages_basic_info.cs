...
namespace PagesBasicInfo
{
    ...
    class PagesBasicInfo
    {
        ...
        private IRestClient client = new RestClient(new RestClientOptions() 
        { MaxTimeout = 60 * 1000, BaseUrl = new System.Uri("https://servicesapi.foxitsoftware.cn/api") });
        ...
        private string PagesBasicInfoTask(string input_file)
        {
            ...
            string url = $"https://servicesapi.foxitsoftware.cn/api/document/pagesBasicInfo?sn={sn}&clientId={client_id}";
            ...
            formData.Add(CreateEncodedFileContent(input_file, "inputDocument", "application/pdf"));
            formData.Add(new StringContent(page_range), "pageRange");
        ...
        private dynamic PollForResult(string task_id, int interval_in_miliseconds = 2000)
        {
            do
            {
                try
        ...
        public static void Start()
        {
            try
            {
                PagesBasicInfo pages_basic_info = new PagesBasicInfo();
                pages_basic_info.GetCredentialsParams(Directory.GetCurrentDirectory() + "/foxit_cloud_api_credentials.json");
                string task_id = pages_basic_info.PagesBasicInfoTask(input_file_path);
                dynamic result = pages_basic_info.PollForResult(task_id);
                if (result == null) {
                    Console.WriteLine("No result returned!");
                    return;
                }
                foreach (var page_info in result)
                {
                    Console.WriteLine("Page Index: {0}, Rotation: {1}, Width: {2}, Height: {3}",
                        page_info.pageIndex, page_info.rotation, page_info.width, page_info.height);
                }
                Console.WriteLine("Get Pages basic information successfully!");
            }
            catch (RestException e)
            {
