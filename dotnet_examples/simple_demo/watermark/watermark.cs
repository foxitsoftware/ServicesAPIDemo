...
namespace WatermarkCS
{
    ...
    class Watermark
    {
        ...
        private const string output_file_path = "../output_files/watermark/Watermark.pdf";
        ...
        public static void Start()
        {
            ...
            try
            {
                ...
                watermark.GetCredentialsParams(Directory.GetCurrentDirectory() + "/foxit_cloud_api_credentials.json");
                string task_id = watermark.WatermarkTask(input_file_path);
                string doc_id = watermark.PollForDocId(task_id);
                watermark.DownLoadFileByDocId(doc_id, output_file_path);
                Console.WriteLine("Add watermark successfully!");
            }
            catch (RestException e)
            {
