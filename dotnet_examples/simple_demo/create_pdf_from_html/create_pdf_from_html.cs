...
// NOTE: before using this demo, you need to add the NuGet package RestSharp 106.10.0 and Newtonsoft.Json latest version.
...
namespace CreatePDFFromHtmlCS
{
  ...
  class CreatePDFFromHtml
  {
    ...
    private const string url = "https://developers.foxitsoftware.cn/version/home-page/";
    private const string output_file_path = "../output_files/create_pdf_from_html/SDKDevelopers.pdf";
    ...
    private string CreatePDFTask(string url, string format = "url")
    {
      var request = new RestRequest("document/createFromHtml", Method.Post);
      request
    ...
    public static void Start()
    {
      ...
      try
      {
        CreatePDFFromHtml create_pdf_from_html = new CreatePDFFromHtml();
        create_pdf_from_html.GetCredentialsParams(Directory.GetCurrentDirectory() + "/foxit_cloud_api_credentials.json");
        string task_id = create_pdf_from_html.CreatePDFTask(url);
        string doc_id = create_pdf_from_html.PollForDocId(task_id);
        create_pdf_from_html.DownLoadFileByDocId(doc_id, output_file_path);
        Console.WriteLine("Create PDF from html successfully!");
      }
      catch(RestException e)
      {
