...
namespace ExtractImageFromPDFCS
{
    ...
    class ExtractImageFromPDF
    {
        ...
        private string ExtractPDFTask(string input_file_path, string mode = "extractImages", string page_range = "")
        {
            ...
            string url = $"https://servicesapi.foxitsoftware.cn/api/document/extract?sn={sn}&clientId={client_id}";
            ...
            formData.Add(new StringContent(mode), "mode");
            if (page_range != "")
        ...
        public static void Start()
        {
            ...
            try
            {
                ExtractImageFromPDF extract_image_from_pdf = new ExtractImageFromPDF();
                extract_image_from_pdf.GetCredentialsParams(Directory.GetCurrentDirectory() + "/foxit_cloud_api_credentials.json");
                string task_id = extract_image_from_pdf.ExtractPDFTask(input_file_path);
                string doc_id = extract_image_from_pdf.PollForDocId(task_id);
                extract_image_from_pdf.DownLoadFileByDocId(doc_id, output_file_path);
                Console.WriteLine("Extract image from PDF successfully!");
            }
            catch (RestException e)
            {
