...
namespace PageIsScannedCS
{
    ...
    class PageIsScanned
    {
        ...
        private const string input_file_path = "../input_files/AboutFoxit_ocr.pdf";
        ...
        private string PageIsScannedTask(string input_file)
        {
            ...
            string url = $"https://servicesapi.foxitsoftware.cn/api/document/pagesIsScanned?sn={sn}&clientId={client_id}";
            ...
            do
            {
                try
                {
                    ...
                    if (percentage == 100)
                    {
                        ...
                        return json.pagesIsScannedResult;
                    }
                }
        ...
        public static void Start()
        {
            try
            {
                PageIsScanned page_is_scanned = new PageIsScanned();
                page_is_scanned.GetCredentialsParams(Directory.GetCurrentDirectory() + "/foxit_cloud_api_credentials.json");
                string task_id = page_is_scanned.PageIsScannedTask(input_file_path);
                dynamic result = page_is_scanned.PollForResult(task_id);
                if (result == null) {
                    Console.WriteLine("No result returned!");
                    return;
                }
                JObject jobject_result = JObject.Parse(result.ToString());

                foreach (var page in jobject_result.Properties())
                {
                    Console.WriteLine($"Page Number: {page.Name}, Is Scanned: {page.Value}");
                }
                Console.WriteLine("Check Scanned Pages successfully!");
            }
            catch (RestException e)
            {
