...
namespace RemovePasswordCS
{
    ...
    class RemovePassword
    {
        ...
        private const string input_file_path = "../input_files/AboutFoxit_123.pdf";
        private const string output_file_path = "../output_files/removepassword/AboutFoxit_RemovePassword.pdf";
        ...
        private string RemovePasswordTask(string input_file_path, string password)
        {
            ...
            string url = $"https://servicesapi.foxitsoftware.cn/api/document/removePassword?sn={sn}&clientId={client_id}";
        ...
        public static void Start()
        {
            ...
            try
            {
                RemovePassword remove_password = new RemovePassword();
                remove_password.GetCredentialsParams(Directory.GetCurrentDirectory() + "/foxit_cloud_api_credentials.json");
                string task_id = remove_password.RemovePasswordTask(input_file_path, "123");
                string doc_id = remove_password.PollForDocId(task_id);
                remove_password.DownLoadFileByDocId(doc_id, output_file_path);
                Console.WriteLine("Remove password successfully!");
            }
            catch (RestException e)
            {
