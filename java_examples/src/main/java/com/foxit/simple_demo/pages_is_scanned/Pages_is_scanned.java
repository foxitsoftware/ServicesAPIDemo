...
//
// This file contains an example to demonstrate how to use Foxit Cloud API to check pdf pages whether is scanned or not.


package com.foxit.simple_demo.pages_is_scanned;
...
public class Pages_is_scanned {
    ...
    private static final String input_file_path = "./input_files/AboutFoxit_ocr.pdf";
    ...
    private String pagesIsScannedTask(String input_file) throws Exception {
        ...
        HttpUrl url = buildURI("document/pagesIsScanned")
    ...
    public static void start() throws Exception {
        Pages_is_scanned pages_is_scanned = new Pages_is_scanned();
        pages_is_scanned.getCredentialsParams("foxit_cloud_api_credentials.json");
        String task_id = pages_is_scanned.pagesIsScannedTask(input_file_path);
		JsonElement result = pages_is_scanned.pollForResult(task_id, 2000);
		if (result.isJsonObject()) {
			JsonObject resultObject = result.getAsJsonObject();
			for (Map.Entry<String, JsonElement> entry : resultObject.entrySet()) {
				String pageNumber = entry.getKey();
				boolean isScanned = entry.getValue().getAsBoolean();
				System.out.println("Page Number: " + pageNumber + ", Scanned: " + isScanned);
			}
		} else {
			System.out.println("Result is not a JSON object.");
		}
        System.out.println("Check scanned pages successfully!");
    }
