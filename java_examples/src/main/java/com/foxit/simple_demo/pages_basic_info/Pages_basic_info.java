...
//
// This file contains an example to demonstrate how to use Foxit Cloud API to get pdf pages's information. 


package com.foxit.simple_demo.pages_basic_info;
...
public class Pages_basic_info {
    ...
    private String pagesBasicInfoTask(String input_file) throws Exception {
        Map<String, String> query_params = new TreeMap<>();
        query_params.put("clientId", client_id);
        query_params.put("pageRange", "all");
        ...
        HttpUrl url = buildURI("document/pagesBasicInfo")
            .addQueryParameter("sn", sn)
            .addQueryParameter("clientId", client_id)
            .build();

        String file_name = (new File(input_file)).getName();
    ...
    private JsonElement pollForResult(String task_id, int interval_in_miliseconds) throws Exception {
        JsonParser parser = new JsonParser();
        do{
            try {
    ...
    public static void start() throws Exception {
        Pages_basic_info pages_basic_info = new Pages_basic_info();
        pages_basic_info.getCredentialsParams("foxit_cloud_api_credentials.json");
        String task_id = pages_basic_info.pagesBasicInfoTask(input_file_path);
		JsonElement result = pages_basic_info.pollForResult(task_id, 2000);
		if (result.isJsonArray()) {
			for (JsonElement pageElement : result.getAsJsonArray()) {
				JsonObject pageObject = pageElement.getAsJsonObject();
				int pageIndex = pageObject.get("pageIndex").getAsInt();
				int rotation = pageObject.get("rotation").getAsInt();
				double width = pageObject.get("width").getAsDouble();
				double height = pageObject.get("height").getAsDouble();
			...
			System.out.println("Result is not a JSON array.");
		}
        System.out.println("Get pages basic info successfully!");
    }
