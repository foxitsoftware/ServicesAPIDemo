...
const inputFilePath = '../input_files/combine.zip'
const outputFilePath = '../output_files/combine/CombineResultFiles.pdf'
...
function combineTask(input_zip_file){
  ...
  formData.append('inputZipDocument', readStream)
...
async function start(){
  ...
  const taskId = await combineTask(inputFilePath)
  const docId = await pollForDocId(taskId)
  await downloadFileByDocId(docId, outputFilePath)
}
 
start().then(function(){
  console.log("Combine PDF files successfully!");
  process.exit(0)
}).catch(function(err){
  console.log("Combine PDF files failed: " + err);
  process.exit(1)
})