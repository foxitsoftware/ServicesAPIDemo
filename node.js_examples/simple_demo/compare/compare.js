...
const inputFilePath1 = '../input_files/test_base.pdf'
const inputFilePath2 = '../input_files/test_compared.pdf'
const outputFilePath = '../output_files/compare/CompareResultFiles.json'
...
function comparePDFTask(inputFilePath1, inputFilePath2){
  const readStream = fs.createReadStream(inputFilePath1)
  ...
  const readStream2 = fs.createReadStream(inputFilePath2)
  ...
  formData.append('inputBaseDocument', readStream)
  formData.append('inputCompareDocument', readStream2)
  formData.append('resultType', 'json')
  formData.append('compareType', 'all')
  ...
  return request({
    ...
    console.log("Compare task error:", err.response.data);
    throw err
  })
}
...
async function start(){
  ...
  const taskId = await comparePDFTask(inputFilePath1, inputFilePath2)
  const docId = await pollForDocId(taskId)
  await downloadFileByDocId(docId, outputFilePath)
}
 
start().then(function(){
  console.log("Compare PDF files successfully!");
  process.exit(0)
}).catch(function(err){
  console.log("Compare PDF files failed: " + err);
  process.exit(1)
})