...
//
// This file contains an example to demonstrate how to use Foxit Cloud API to protect pdf file.
...
const outputFilePath = '../output_files/protect/AboutFoxitPassword123.pdf'
...
function protectPDFTask(inputFilePath){
  let passwordProtection = {
    userPassword : '123',
    ownerPassword: '456'
  }
  const passwordString = JSON.stringify(passwordProtection);

  let permission = ['EDIT_ANNOTATION', 'EDIT_CONTENT']
  const permissionString =  JSON.stringify(permission);

  const encryptionAlgorithm = 'AES_128';
  const querystring = require('querystring');
  const crypto = require('crypto'); 
  let queryParams = {
  'clientId': clientId,
  'passwordProtection': passwordString,
  ...
  formData.append('passwordProtection', passwordString)
  ...
  return request({
    ...
    console.log("Protect pdf task error:", err.response.data);
    throw err
  })
}
...
async function start(){
  ...
  const taskId = await protectPDFTask(inputFilePath)
  const docId = await pollForDocId(taskId)
  await downloadFileByDocId(docId, outputFilePath)
}
 
start().then(function(){
  console.log("Protect PDF file successfully!");
  process.exit(0)
}).catch(function(err){
  console.log("Protect PDF file failed: " + err);
  process.exit(1)
})