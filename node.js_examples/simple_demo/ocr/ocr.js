...
const outputFilePath = '../output_files/ocr/AboutFoxit_ocr.txt'
...
async function start(){
  ...
  const taskId = await ocrTask(inputFilePath)
  const docId = await pollForDocId(taskId)
  await downloadFileByDocId(docId, outputFilePath)
}
 
start().then(function(){
  console.log("OCR PDF file successfully!");
  process.exit(0)
}).catch(function(err){
  console.log("OCR PDF file failed: " + err);
  process.exit(1)
})