...
//
// This file contains an example to demonstrate how to use Foxit Cloud API to convert pdf other format files to pdf files.
...
const outputFilePath = '../output_files/convert_pdf_to_others/Image.zip'
...
function convertPDFTask(inputFilePath, format = 'image'){
  let config = {
    dpi : 96,
    pageRange : 'all'
  }
  ...
  return request({
    ...
    console.log("Convert pdf to others task error:", err.response.data);
    throw err
  })
}
...
async function start(){
  ...
  const taskId = await convertPDFTask(inputFilePath)
  const docId = await pollForDocId(taskId)
  await downloadFileByDocId(docId, outputFilePath)
}
 
start().then(function(){
  console.log("Convert PDF to image successfully!");
  process.exit(0)
}).catch(function(err){
  console.log("Convert PDF to image failed: " + err);
  process.exit(1)
})