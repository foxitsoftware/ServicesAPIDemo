...
const outputFilePath = '../output_files/linearize/Linearize.pdf'
...
function linearizePDFTask(inputFilePath){
  ...
  return request({
    ...
    console.log("Linearize task error:", err.response.data);
    throw err
  })
}
...
async function start(){
  ...
  const taskId = await linearizePDFTask(inputFilePath)
  const docId = await pollForDocId(taskId)
  await downloadFileByDocId(docId, outputFilePath)
}
 
start().then(function(){
  console.log("Linearize PDF file successfully!");
  process.exit(0)
}).catch(function(err){
  console.log("Linearize PDF file failed: " + err);
  process.exit(1)
})