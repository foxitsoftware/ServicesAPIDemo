...
function createPDFTask(url, format = 'url'){
  let config = {
    width: 640,
    height : 900,
    rotate : 0,
  ...
  return request({
    ...
    console.log("Create pdf from html task error:", err.response.data);
    throw err
  })
}
...
async function start(){
  ...
  const taskId = await createPDFTask(url)
  const docId = await pollForDocId(taskId)
  await downloadFileByDocId(docId, outputFilePath)
}
 
start().then(function(){
  console.log("Create PDF from html successfully!");
  process.exit(0)
}).catch(function(err){
  console.log("Create PDF from html failed: " + err);
  process.exit(1)
})