...
const inputFilePath = '../input_files/AboutFoxit_123.pdf'
const outputFilePath = '../output_files/removepassword/AboutFoxit_RemovePassword.pdf'
...
async function start(){
  ...
  const taskId = await removePasswordTask(inputFilePath, '123')
  const docId = await pollForDocId(taskId)
  await downloadFileByDocId(docId, outputFilePath)
}
 
start().then(function(){
  console.log("Remove Password for PDF file successfully!");
  process.exit(0)
}).catch(function(err){
  console.log("Remove Password for PDF file failed: " + err);
  process.exit(1)
})