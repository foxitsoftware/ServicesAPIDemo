...
const outputFilePath = '../output_files/manipulation/Manipulation.pdf'
...
async function start(){
  ...
  const taskId = await manipulationPDFTask(inputFilePath)
  const docId = await pollForDocId(taskId)
  await downloadFileByDocId(docId, outputFilePath)
}
 
start().then(function(){
  console.log("Manipulate PDF successfully!");
  process.exit(0)
}).catch(function(err){
  console.log("Manipulate PDF failed: " + err);
  process.exit(1)
})