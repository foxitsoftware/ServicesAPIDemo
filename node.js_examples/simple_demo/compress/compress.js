...
const inputFilePath = '../input_files/original1.pdf'
const outputFilePath = '../output_files/compression/CompressionResultFile.pdf'
...
function compressPDFTask(inputFilePath){
  ...
  formData.append('compressionLevel', 'high');

  //Upload a file and create a new workflow task.
  return request({
    method: 'post',
    url: '/document/compress',
    ...
    console.log("Compress task error:", err.response.data);
    throw err
  })
}
...
async function start(){
  ...
  const taskId = await compressPDFTask(inputFilePath)
  const docId = await pollForDocId(taskId)
  await downloadFileByDocId(docId, outputFilePath)
}
 
start().then(function(){
  console.log("Compress PDF file successfully!");
  process.exit(0)
}).catch(function(err){
  console.log("Compress PDF file failed: " + err);
  process.exit(1)
})