...
class ExtractImageFromPDF:
    def __init__(self):
        ...
        self.input_file_path = '../input_files/PDF2Img.pdf'
        self.output_file_path = '../output_files/extract_image_from_pdf/Image.zip'
    ...
    def extract_image_from_pdf_task(self, input_file, mode = "extractImages", page_range = "all"):
        queryParams = {
        ...
        response = requests.request("POST", self.build_uri('document/extract'), 
    ...
    def start(self):
        try:
            ...
            task_id = self.extract_image_from_pdf_task(self.input_file_path)
            doc_id = self.poll_for_docid(task_id)
            self.down_load_file_by_docid(doc_id, self.output_file_path)
            print('Extract image from PDF successfully!')
