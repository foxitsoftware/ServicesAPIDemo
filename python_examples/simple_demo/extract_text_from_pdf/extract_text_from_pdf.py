...
class ExtractTextFromPDF:
    def __init__(self):
        ...
        self.output_file_path = '../output_files/extract_text_from_pdf/AboutFoxit.txt'
    ...
    def start(self):
        try:
            ...
            task_id = self.extract_text_from_pdf_task(self.input_file_path)
            doc_id = self.poll_for_docid(task_id)
            self.down_load_file_by_docid(doc_id, self.output_file_path)
            print('Extract text from PDF successfully!')
