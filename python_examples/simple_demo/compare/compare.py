...
class Compare:
    def __init__(self):
        ...
        self.input_file_path1 = '../input_files/test_base.pdf'
        self.input_file_path2 = '../input_files/test_compared.pdf'
        self.output_file_path = '../output_files/compare/CompareResultFiles.json'
    ...
    def compare_task(self, input_file_base, input_file_compare):
        resultTypeString = "json"
        compareTypeString = "all"

        queryParams = {
            'clientId': self.client_id,
            'resultType': resultTypeString, 
            'compareType': compareTypeString
        }
        ...
        payload = {'resultType': resultTypeString, 'compareType': compareTypeString}
        filename_base = os.path.basename(input_file_base)
        filename_compare = os.path.basename(input_file_compare)
        files = {
            ('inputBaseDocument', (filename_base, open(input_file_base,'rb'), 'application/pdf')),
            ('inputCompareDocument', (filename_compare, open(input_file_compare,'rb'), 'application/pdf')),
            }
        ...
        response = requests.request("POST", self.build_uri('document/compare'), 
    ...
    def start(self):
        try:
            ...
            task_id = self.compare_task(self.input_file_path1, self.input_file_path2)
            doc_id = self.poll_for_docid(task_id)
            self.down_load_file_by_docid(doc_id, self.output_file_path)
            print('Compare PDF files successfully!')
