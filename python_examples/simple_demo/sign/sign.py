...
class Sign:
    def __init__(self):
        ...
        self.input_image_file_path = '../input_files/Image.jpg'
        self.output_file_path = '../output_files/sign/AboutFoxitSign.pdf'
    ...
    def sign_task(self, input_file, input_image_file):
        pageIndex = 1;
        rect = "[0, 0, 200, 200]";
        ...
        payload = {'certId': self.certId, 'pageIndex': pageIndex, 'signatureRect': rect,'appearanceFlag': appearanceFlag,'signer': signer,'text': text,'location': location,'reason': reason,'contactInfo': contactInfo}
        filename = os.path.basename(input_file)
        files = {
            ('inputDocument', (filename, open(input_file,'rb'), 'application/pdf')),
            ('stampImage', (filename, open(input_image_file,'rb'), 'application/jpg'))
            }
        ...
        response = requests.request("POST", self.build_uri('document/sign'), 
    ...
    def start(self):
        try:
            ...
            task_id = self.sign_task(self.input_file_path, self.input_image_file_path)
            doc_id = self.poll_for_docid(task_id)
            self.down_load_file_by_docid(doc_id, self.output_file_path)
            print('Sign PDF successfully!')
